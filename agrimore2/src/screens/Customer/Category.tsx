...
export default function Category({ route, navigation }: any) {
  ...
  const loadCategoryProducts = async (catId: string) => {
    ...
    const allProducts = snap.docs.map(d => ({ id: d.id, ...d.data() })).filter((p:any) => p.isAvailable !== false);
    
    // Support matching by name, id, or slug
    const filtered = allProducts.filter((p: any) => {
      const pCat = (p.categoryName || p.category || p.categoryId || '').toString().toLowerCase();
      const matchName = cat.name?.toLowerCase();
      const matchId = cat.id?.toLowerCase();
      const matchSlug = cat.slug?.toLowerCase();
      
      return pCat === matchName || pCat === matchId || (matchSlug && pCat === matchSlug);
    });
