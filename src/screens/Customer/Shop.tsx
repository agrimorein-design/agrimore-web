...
export default function Shop({ navigation }: any) {
  ...
  const filteredProducts = products.filter(p => {
    if (selectedCategory !== 'All') {
      const matchCat = categories.find(c => c.name === selectedCategory);
      const pCat = (p.categoryName || p.category || p.categoryId || '').toString().toLowerCase();
      
      if (matchCat) {
        const matchName = matchCat.name?.toLowerCase();
        const matchId = matchCat.id?.toLowerCase();
        const matchSlug = matchCat.slug?.toLowerCase();
        
        if (pCat !== matchName && pCat !== matchId && pCat !== matchSlug) {
          return false;
        }
      } else {
